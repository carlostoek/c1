# PROMPT G3.4: ConfigurationOrchestrator - Coordinador Principal

---

## ROL

Actúa como Ingeniero de Software Senior especializado en arquitecturas de orquestación jerárquica y workflows complejos multi-entidad.

---

## TAREA

Implementa `ConfigurationOrchestrator` en `bot/gamification/services/orchestrator/configuration.py`, el orquestador maestro que coordina `MissionOrchestrator` y `RewardOrchestrator` para crear configuraciones completas de gamificación en una transacción.
...
El wizard del diseño permite crear:
- 1 Misión
- 1 Nivel nuevo (auto level-up)
- 1+ Recompensas nuevas

Todo debe crearse en **una sola transacción atómica**.

---

## RESPONSABILIDADES

### 1. Creación Completa de Sistema de Misión
...
async def create_complete_mission_system(
    ...
    """
    Crea sistema completo: misión + nivel + recompensas.
    ...
    Flujo:
    1. Validar config completa
    2. Llamar a mission_orchestrator.create_mission_with_dependencies()
    3. Si falla, rollback completo
    4. Retornar resumen
    """
```

### 2. Aplicación de Plantilla Completa
...
async def apply_system_template(
    ...
    - "progression_system": 5 niveles + badges por nivel
...
### 3. Validación Cross-Entity

```python
async def validate_complete_config(config: dict) -> tuple[bool, List[str]]:
    """
...
```python
# bot/gamification/services/orchestrator/configuration.py
...
class ConfigurationOrchestrator:
    ...
    async def validate_complete_config(self, config: dict) -> tuple[bool, List[str]]:
        ...
        if config.get('auto_level'):
            mission_besitos = config['mission']['besitos_reward']
            level_min = config['auto_level']['min_besitos']
...
@property
def configuration_orchestrator(self) -> ConfigurationOrchestrator:
...
**ENTREGABLE:** Archivo `configuration.py` con orquestador maestro.
