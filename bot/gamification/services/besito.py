...
class BesitoService:
    ...
    async def get_balance(self, user_id: int) -> float:
        ...
        """
        # Primero intentar obtener sin crear
    ...
    async def _get_or_create_user_gamification(self, user_id: int) -> UserGamification:
        ...
        Note: Si el perfil no existe, se crea pero NO se hace commit aquí.
        El caller es responsable de manejar la transacción.
        ...
        if not user_gamif:
            user_gamif = UserGamification(user_id=user_id)
            self.session.add(user_gamif)
            # Flush para obtener el ID sin hacer commit (caller maneja transacción)
            await self.session.flush()
